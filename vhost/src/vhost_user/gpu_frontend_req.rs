...
impl FrontendInternal {
    ...
    fn send_header(&mut self, request: GpuBackendReq) -> io::Result<()> {
        self.check_state()?;

        let hdr = VhostUserGpuMsgHeader::new(request, VhostUserGpuHeaderFlag::REPLY.bits(), 0);
        self.sock
            .send_header(&hdr, None)
            .map_err(io::Error::other)?;
        Ok(())
    }
}
...
impl GpuFrontend {
    ...
    /// Send a header-only response (empty body, size=0).
    /// Used for ACK responses like DMABUF_UPDATE.
    pub fn send_header(&self, request: GpuBackendReq) -> io::Result<()> {
        let mut node = self.node();
        node.send_header(request)?;
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_send_header_ack() {
        let (frontend, mut backend) = frontend_backend_pair();

        let sender = thread::spawn(move || {
            let hdr = VhostUserGpuMsgHeader::new(GpuBackendReq::DMABUF_UPDATE, 0, 0);
            backend.send_header(&hdr, None).unwrap();

            let (reply_hdr, _fds) = backend.recv_header().unwrap();
            assert!(reply_hdr.is_reply());
            assert_eq!(reply_hdr.get_code().unwrap(), GpuBackendReq::DMABUF_UPDATE);
            assert_eq!(reply_hdr.get_size(), 0);
        });

        let (req, payload, _fds) = frontend.read_message().unwrap();
        assert_eq!(req, GpuBackendReq::DMABUF_UPDATE);
        assert!(payload.is_empty());

        frontend.send_header(req).unwrap();
    ...
    fn test_large_payload() {
        ...
        // Simulate a large gfxstream framebuffer (1024x768x4 bytes)
        let payload_size = 1024 * 768 * 4;
        let large_payload = vec![0x42u8; payload_size];
        let expected_payload = large_payload.clone();

        let sender = thread::spawn(move || {
            let hdr = VhostUserGpuMsgHeader::new(GpuBackendReq::UPDATE, 0, payload_size as u32);
            backend.send_header(&hdr, None).unwrap();
            // Send payload in chunks to simulate partial writes
            use std::io::Write;
            backend
                .try_clone_sock()
                .unwrap()
        ...
        let (req, payload, _fds) = frontend.read_message().unwrap();
        assert_eq!(req, GpuBackendReq::UPDATE);
    ...
    #[test]
    fn test_send_header_when_failed() {
        let (fe, _be) = UnixStream::pair().unwrap();
        let frontend = GpuFrontend::from_stream(fe);

        frontend.set_failed(libc::EIO);

        // send_header should fail when frontend is in error state
        assert!(frontend.send_header(GpuBackendReq::DMABUF_UPDATE).is_err());
    }
}
